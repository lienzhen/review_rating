...
def gradedecentRatingItem(data):
    ...
    for itemId in data.scoreItemDic:
        grad = np.zeros((1,data.CONST_K))
        for (userId,score) in data.scoreItemDic[itemId].items():
            v = np.dot(data.userV[userId],data.itemV[itemId]) + data.globalBaise + data.userBaise[userId] + data.itemBaise[itemId] + np.dot(data.user_vec[data.idToUserDic[userId]], data.user_weight) + np.dot(data.item_vec[data.idToItemDic[itemId]], data.item_weight) - score
...
def gradedecentBaise(data):
    ...
    for userId,arr in data.scoreUserDic.items():
        gradU = 0.0
        for itemId,score in arr.items():
            gradU += np.dot(data.userV[userId],data.itemV[itemId]) + data.globalBaise + data.userBaise[userId] + data.itemBaise[itemId] + np.dot(data.user_vec[data.idToUserDic[userId]], data.user_weight) + np.dot(data.item_vec[data.idToItemDic[itemId]], data.item_weight) - score
    ...
    for itemId,arr in data.scoreItemDic.items():
        ...
        for userId,score in arr.items():
            gradV += np.dot(data.userV[userId],data.itemV[itemId]) + data.globalBaise + data.userBaise[userId] + data.itemBaise[itemId] + np.dot(data.user_vec[data.idToUserDic[userId]], data.user_weight) + np.dot(data.item_vec[data.idToItemDic[itemId]], data.item_weight) - score
